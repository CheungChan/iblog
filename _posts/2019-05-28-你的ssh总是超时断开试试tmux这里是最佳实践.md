...
![image-20190528150058804](/imgbak/image-20190528150058804.png)

### Tmux可以解决这一切烦恼

tmux 是一款终端复用命令行工具，一般用于 Terminal 的窗口管理。在 macOS 下，使用 iTerm2 能应付绝大多数需求。

![image-20190528131724179](/imgbak/image-20190528131724179.png)
...
![image-20190528140500234](/imgbak/image-20190528140500234.png)
...
- jump跳板机: tmux创建并打开一个新的tmux session. 然后ssh连接对应的服务器, 使用`^b+,`重命名为服务器的简称,使用`^b+c`新建window, 再做用样的操作即可. 可以通过`^b + $` 重命名session为`server`, 以后也是通过`tmux a`直接进入环境
  ...
  ![image-20190528151120270](/imgbak/image-20190528151120270.png)
